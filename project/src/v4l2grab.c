...
/*	process image read*/
static void imageProcess(const void* p, struct timeval timestamp, int new_socket)
{
	//timestamp.tv_sec
	//timestamp.tv_usec
	...
	int j;
	int nbBytesSend;
	int nb_msg = 24; 					//nb bytes envoyees a la fois
	int len_msg = width*height*3/nb_msg; 	//nb de msgs a envoyer
	printf("len msg : %d, nb : %d, tt : %d\n", len_msg, nb_msg, len_msg*nb_msg);

	char buff2[7*sizeof(char)];
	char buff3[(len_msg*8)*sizeof(char)];
	sprintf(buff3, "%d;%d;", width, height);
	...
	//si msg client -> envoi
	if (msg == 1){	
		for (i=0; i<nb_msg/3; i++){
			
			//creation packet
			for (j=0; j<len_msg; j++){
				sprintf(buff2, "%d;", *(src+(i*len_msg+j)));
				strcat(buff3, buff2);
			...
			//envoi packet
			nbBytesSend = 0;				
		 	nbBytesSend = send(new_socket , buff3 , strlen(buff3) , MSG_NOSIGNAL ); //strlen(buff3)
		 	printf("%d, len buff3 : %d, val %c %c\n ", i, strlen(buff3), *buff3,*(buff3+1));
...
/*	read single frame */
static int frameRead(int new_socket)
{
	...
	switch (io) {
		...
		case IO_METHOD_READ:
			...
			imageProcess(buffers[0].start,timestamp, new_socket);
			break;
#endif
		...
		case IO_METHOD_MMAP:
			...
			imageProcess(buffers[buf.index].start,buf.timestamp, new_socket);
			...
			case IO_METHOD_USERPTR:
				...
				imageProcess((void *)buf.m.userptr,buf.timestamp, new_socket);
...
static void mainLoop(int new_socket)
{	
	int count;
...
{	
	...
	syslog(LOG_DEBUG, "This is a try...");
  	closelog();
